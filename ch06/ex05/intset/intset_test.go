...
func TestIntSet_IntersectWith(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			name:     "{1} and {1, 64} = {1}",
			s:        NewIntSet(1),
			other:    NewIntSet(1, 64),
			expected: NewIntSet(1),
		},
		{
			name:     "{1, 64} and {1, 64} = {1, 64}",
			s:        NewIntSet(1, 64),
			other:    NewIntSet(1, 64),
			expected: NewIntSet(1, 64),
		},
	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			testCase.s.IntersectWith(testCase.other)
			assert.True(
				t,
				testCase.s.Equals(testCase.expected),
				"expected: %v, but got %v",
...
func TestIntSet_DifferenceWith(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			s:        NewIntSet(2, 65),
			other:    NewIntSet(1, 64),
			expected: NewIntSet(2, 65),
		},
	}
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			testCase.s.DifferenceWith(testCase.other)
			assert.True(
				t,
				testCase.s.Equals(testCase.expected),
				"expected: %v, but got %v",
...
func TestIntSet_SymmetricDifference(t *testing.T) {
	...
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			testCase.s.SymmetricDifference(testCase.other)
			assert.True(
				t,
				testCase.s.Equals(testCase.expected),
				"expected: %v, but got %v",
...
func TestIntSet_Elems(t *testing.T) {
	testCases := []struct {
		...
		{
			name:     "空の IntSet の場合, 空スライスを返却",
			s:        NewIntSet(),
			expected: []uint{},
		},
		{
			name:     "IntSet の要素が 1 の場合, 要素に 1 を持つスライスを返却",
			s:        NewIntSet(1),
			expected: []uint{1},
		},
		{
			name:     "IntSet の要素が 2 の場合, 要素に 2 を持つスライスを返却",
			s:        NewIntSet(2),
			expected: []uint{2},
		},
		{
			name:     "IntSet の要素が 64 の場合, 要素に 64 を持つスライスを返却",
			s:        NewIntSet(64),
			expected: []uint{64},
		},
		{
			name:     "IntSet の要素が 1, 2 の場合, 要素に 1, 2 を持つスライスを返却",
			s:        NewIntSet(1, 2),
			expected: []uint{1, 2},
		},
		{
			name:     "IntSet の要素が 1, 64 の場合, 要素に 1, 64 を持つスライスを返却",
			s:        NewIntSet(1, 64),
			expected: []uint{1, 64},
		},
	}
